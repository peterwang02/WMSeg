...
################### MODIFIED #################
def train(args):
    with open("testEval.txt", "w") as out:
        out.write("Test Evaluation: \n")
    ...
    output_model_dir = os.path.join('./models', args.model_name)

    word2id = get_word2id(args.train_data_path)

    if args.use_memory:
        gram2id, gramfreq, av = get_gram2id(args.train_data_path, args.eval_data_path)
    else:
        gram2id = None
        gramfreq = None
        av = None
    ...
    seg_model = WMSeg(word2id, gram2id, gramfreq, av, label_map, hpara, args)

    # Examples are just sentences in the file!!!!!!!!!!!!!!!!!!!
    ...
    if args.do_train:
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, start_index in enumerate(tqdm(range(0, len(train_examples), args.train_batch_size), position=0, leave=True)):
                seg_model.train()
                ...
                input_ids, input_mask, l_mask, label_ids, matching_matrix, \
                segment_ids, valid_ids, word_ids, word_mask, freq, avfreq = feature2input(device, train_features)

                loss, _ = seg_model(input_ids, segment_ids, input_mask, label_ids, valid_ids, l_mask, word_ids,
                                    matching_matrix, word_mask, freq, avfreq)
                if n_gpu > 1:
            ...
            if args.local_rank == -1 or torch.distributed.get_rank() == 0:
                ...
                for start_index in range(0, len(eval_examples), args.eval_batch_size):
                    ...
                    input_ids, input_mask, l_mask, label_ids, matching_matrix, \
                    segment_ids, valid_ids, word_ids, word_mask, freq, avfreq = feature2input(device, eval_features)
                    ...
                    with torch.no_grad():
                        _, tag_seq = seg_model(input_ids, segment_ids, input_mask, labels=label_ids,
                                                   ...
                                                   word_mask=word_mask, freq=freq, avfreq=avfreq)
                ...
                p, r, f = cws_evaluate_word_PRF(y_pred_all, y_true_all, "testEval.txt")
                oov = cws_evaluate_OOV(y_pred, y_true, sentence_all, word2id, "testEval.txt")
                ...
                if args.model_name is not None:
                    ...
                    if args.model_name:
                        with open(os.path.join(output_model_dir, 'CWS_result.txt'), "w", encoding='utf-8') as writer:
...
def test(args):
    ...
    y_pred = []
    with open("testEval.txt", "w") as out:
        out.write("Evaluation of Test: \n")
    for start_index in tqdm(range(0, len(eval_examples), args.eval_batch_size), position=0, leave=True):
        eval_batch_examples = eval_examples[start_index: min(start_index + args.eval_batch_size,
                                                             len(eval_examples))]
        eval_features = convert_examples_to_features(eval_batch_examples)
...
def main():
    ...
    parser.add_argument("--use_memory",
                        action='store_true',
                        help="Whether to use kv-memory on BERT.")    
